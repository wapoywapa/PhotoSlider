...
public class ViewController: UIViewController {
    ...
    public override func viewDidLoad() {
        ...
        for imageResource in imageResources()! {
            ...
            //CUSTOM
            var isPrivateRequest = false
            if imageResource is URL
            {
                if let urlString = imageResource.absoluteString
                {
                    if urlString?.range(of: "private") != nil
                    {
            ...
            {
                imageView = PhotoSlider.ImageView(frame: frame)
            }
